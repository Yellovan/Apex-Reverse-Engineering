...
**Resolved 2026-07-30 — inconsistent timeframe references are not actually inconsistent.** The same EA/account previously showed three different timeframe tags across evidence: `(XAUUSD.sc,M30)` in E-017/E-018's terminal lines, `(XAUUSD.sc,H1)` in E-020's Experts-tab lines, and an `H4` chart in E-022's `h4.png`. Melvin confirmed 2026-07-30: Apex trades on multiple timeframes simultaneously by design, so the specific chart period an instance happens to be attached to (M30/H1/H4/M15, all confirmed live) doesn't matter to its behaviour — this is option (a) from the three candidates below, generalized: it's not "only one instance is really trading," it's that the underlying logic is timeframe-agnostic and multiple concurrent instances are all genuinely active. This also matches what the reconstruction (`ea_drafts/ZennApex_XAU_v2.mq5`) independently found necessary: its spike-detection had to be decoupled from the chart-period gate entirely to behave correctly (see the EA's 2026-07-30 commit history).
