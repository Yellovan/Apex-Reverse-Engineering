...
| E-017 | Journal | `data/journals/ultima raport.txt` | 2026.07.20–2026.07.24 (one live trading week) | XAUUSD.sc | **Live-money MT5 terminal log, account 31599933, broker "Ultima Markets Ltd" ("UltimaMarkets-Live 2"), hedging mode, VPS-hosted (Hong Kong 13).** Supersedes the withdrawn E-014 as the correct evidence for a live Apex deployment — **directly confirms the EA name/version in the log itself**: `expert ZennbotApex2.3beta1 (XAUUSD.sc,M30)` running through 2026.07.24 12:28, then removed and reloaded as `expert ZennbotApex2.4 (XAUUSD.sc,M30)` at 16:43 the same day — a live version upgrade captured mid-week. A second EA, `testdaashboard (XAUUSD.sc,M1)`, runs alongside Apex (unclear purpose — likely a monitoring/dashboard tool, not part of Apex's own logic). **Note:** stored in the repo as a curated excerpt (see the file's own header) — Melvin pasted the full week's log in conversation, but only representative sections were kept due to output-size limits transcribing it; ask Melvin to re-supply as a file upload if exhaustive parsing of the full week is needed. | H-003, H-006 |
